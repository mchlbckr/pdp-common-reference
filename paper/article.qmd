...
abstract: |
  ...
  support restriction reverses a near-zero random-forest Longitude contrast and
  attenuates the boosting contrast on an effective subpopulation of three to
  six per cent of blocks, with the attenuation depending on the declared
  validity rule by a factor of two. A controlled Bank Marketing stress test
  shows that ordinary partial dependence can change under modifications
  confined to rejected queries. Curves supported entirely on accepted queries
  are invariant under that perturbation; the relaxed curve, which retains mass
  outside the common valid set, is not.
keywords:
  - partial dependence
  - extrapolation
  - support
...
The example also exposes a dependence that the general formulation carries.
Feasibility here requires $\epsilon\geq1/8$, the reciprocal of the number of
groups, and in general the admissible tolerance depends on $K$, that is, on how
finely the displayed range is represented. A grid is an analyst's choice, so a
construction of this kind can be made feasible or infeasible by refining it.
The online supplement shows that for interval-valued validity rules the
constraint set reduces exactly to a finite one and the dependence disappears.
The rotating construction is deliberately not of that form, which is what makes
it a clean separating example and at the same time the case where the
representation matters. Analyses should therefore declare the grid before
inspecting curves and report the minimum feasible tolerance, as done throughout
@sec-experiments.
